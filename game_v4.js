// --- tiny on-screen error so we see issues ---
...
function updateTimerView(){
  ...
  if (E.time) E.time.textContent = fmt(S.t.running ? L : tot);
  if (E.prog) E.prog.value = Math.max(0, Math.min(1, (tot-L)/tot));
  if (E.phase) E.phase.textContent = (S.t.mode==='focus'?'Focus phase':'Break phase') + ' • Ends at ' +
