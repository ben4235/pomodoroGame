// Show errors on-screen so we catch issues fast
...
const ALLOW_SHOP_WHEN_PAUSED = true;
...
function setMode(m){ S.t.mode = m; if (E.pill) E.pill.textContent = (m==='focus'?'Focus':m==='short'?'Break':'Long Break')+' • '+fmt(msFor(m)); }
function schedule(ms){ S.t.endAt = now()+ms; S.t.running=true; S.t.last=now(); S.t.remain=0; persist(); pulse(); }
function completePhase(){ S.t.running=false; if(S.t.mode==='focus'){ S.t.done++; S.streak++; setMode((S.t.done % S.t.goal === 0) ? 'long' : 'short'); } else { setMode('focus'); } persist(); schedule(msFor(S.t.mode)); }
